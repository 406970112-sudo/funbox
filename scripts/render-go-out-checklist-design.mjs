...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/go-out-checklist-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-go-out-checklist-"));
...
for (let i = 0; i < 40; i++) {
  const ready = await send("Runtime.evaluate", {
    expression: `document.querySelectorAll('.phone-wrap').length`,
    returnByValue: true,
  });
  if (ready.result.value === 6) break;
  await sleep(500);
}
...
const layout = await send("Runtime.evaluate", {
  expression: `(() => {
    ...
    const phones = wraps.map((wrap, index) => {
      ...
      const content = wrap.querySelector('.screen-body');
...
writeFileSync(path.join(outDir, "go-out-checklist-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["home", "complete", "scenes", "weather", "reminder", "history"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `go-out-checklist-product-design-v1-${labels[phone.index]}-mobile.png`),
