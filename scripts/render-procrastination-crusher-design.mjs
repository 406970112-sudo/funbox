...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/procrastination-crusher-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-procrastination-crusher-"));
...
const layout = await send("Runtime.evaluate", {
  expression: `(() => {
    const phones = [...document.querySelectorAll('.phone')].map((phone, index) => {
      ...
      return {
        ...
        bodyOverflow: body.scrollHeight > body.clientHeight + 2,
...
writeFileSync(path.join(outDir, "procrastination-crusher-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["home", "split", "execute", "ledger"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `procrastination-crusher-${labels[phone.index]}-mobile.png`),
