...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/parking-location-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9400 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-parking-location-"));
  ...
  expression: `(() => {
    ...
    const phones = wraps.map((wrap, index) => {
      ...
      return {
        ...
        overflow,
        iconCount: wrap.querySelectorAll('i[data-lucide]').length,
        svgCount: wrap.querySelectorAll('svg.lucide').length,
      };
    });
...
writeFileSync(path.join(outDir, "parking-location-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["home", "record", "detail", "reminder", "leave", "history"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `parking-location-product-design-v1-${labels[phone.index]}-mobile.png`),
