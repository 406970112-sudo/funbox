...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/leftover-manager-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-leftover-manager-"));
...
writeFileSync(path.join(outDir, "leftover-manager-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["empty", "home", "add", "detail", "suggestion", "history"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `leftover-manager-product-design-v1-${labels[phone.index]}-mobile.png`),
