...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/time-capsule-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-time-capsule-"));
...
writeFileSync(path.join(outDir, "time-capsule-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["home", "create-personal", "create-goal", "joint", "sealed", "opened"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `time-capsule-product-design-v1-${labels[phone.index]}-mobile.png`),
