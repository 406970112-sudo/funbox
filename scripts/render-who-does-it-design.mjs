...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/who-does-it-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-who-does-it-"));
...
writeFileSync(path.join(outDir, "who-does-it-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["search", "empty", "add", "wheel", "result", "history"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `who-does-it-product-design-v1-${labels[phone.index]}-mobile.png`),
