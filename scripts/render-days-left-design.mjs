...
const boardFile = "C:/Users/Administrator/Documents/funbox/docs/days-left-product-design-v1.html";
const outDir = "C:/Users/Administrator/Documents/funbox/docs";
const boardUrl = `file:///${boardFile.replace(/\\/g, "/")}`;
const port = 9300 + Math.floor(Math.random() * 500);
const userData = mkdtempSync(path.join(tmpdir(), "edge-days-left-"));
...
writeFileSync(path.join(outDir, "days-left-product-design-v1.png"), Buffer.from(full.data, "base64"));

const labels = ["empty", "home", "add", "detail", "categories", "reminders", "list"];
for (const phone of layout.result.value.phones) {
  ...
  writeFileSync(
    path.join(outDir, `days-left-product-design-v1-${labels[phone.index]}-mobile.png`),
