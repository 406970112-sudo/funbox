...
首次启动会自动创建这三个位置。本地开发必须完整保留 `data/` 目录；生产部署脚本会将数据库、头像和 JWT 密钥重定向到固定的 `/srv/my-first-expo-app-shared/`，避免切换 release 时丢失数据。如果 JWT 密钥丢失，现有登录令牌会全部失效。生产环境也可以通过 `AUTH_JWT_SECRET` 直接提供至少 32 个字符的固定密钥。
